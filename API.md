...
The ember API was updated in January 2021 and users are slowly being moved to the new API. The document describing the old API from 2020 is available from [here](API_2020.md)
...
| 2     | 2              | Temperature (read-only)           |
| 4     | 2              | Temperature Data (read-write)     |
| 5     | 4              | Epoch Timestamp                   |
...
| 17    | Setpoint (Auto Mode) | 4    | temp × 10                      | VERIFY    |  R/W   |
...
# MQTT Command Patterns (December 2025 update)

This section documents confirmed MQTT command patterns based on reverse-engineering.
...
**MQTT Downlink (multiple points bundled):**
```
...
# Schedule Encoding (December 2025 update)
...
When `startTime == endTime`, the period is considered disabled/empty. This is how the app represents "No P1", "No P2", or "No P3".
...
| Transport | Use Case |
|-----------|----------|
| HTTP | Authentication, reading zone data, configuration |
| MQTT | Real-time control commands and state updates |

**Key Insight:** HTTP is authoritative for commands; MQTT is reactive for state propagation. pyephember2 works without subscribing to MQTT because subscription is only required for real-time state updates, not for control.
